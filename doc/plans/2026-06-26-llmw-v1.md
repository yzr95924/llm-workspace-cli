...
**2026-06-27 · 任务 19 E2E 暴露 cli.py 全局 flag 顺序 bug（已修）。** 经 `cli.main` 驱动真实 argparse 的 E2E 发现：全局 flag 只挂主 parser，`llmw init -w X`（spec §3.1 写法，flag 在子命令后）被 argparse 拒掉。改为经共享 parents parser 同时挂主 parser 与各子 parser，两种顺序都可用；`default=SUPPRESS` 防 clobber。已单独成 commit `fix(cli)`。

**2026-06-27 · 任务 20 CI 矩阵取 [3.7, 3.11]（偏离计划的 [3.6, 3.11]）。** 3.6 已 EOL（2021-12），现代 runner 镜像无预装、`setup-python@v5` 在 ubuntu-latest 上装不上。选计划预备的方案 (b)：矩阵降到 `["3.7", "3.11"]`。代码按 py37 语法写成（`Optional/List/Dict` 注解、`universal_newlines`），3.7 即证明后向兼容，3.11 覆盖现代。另把单 job 拆成 **lint job(3.11) + test matrix(3.7/3.11)**：test job 不装 `[dev]`（含 ruff，在 3.7 上无法干净安装、会慢回溯到旧 wheel），改为 `pip install -e . "pytest>=7,<8" "pytest-cov>=4"`（钉 pytest<8 因 pytest 8 需 py≥3.8）；lint 由 3.11 job 覆盖。`requires-python = ">=3.6"` 保持不变（代码确为 3.6 安全，只是 CI 测不到 3.6）。
