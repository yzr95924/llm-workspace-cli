...
- **不传 model**（Phase 1 简化版）：
  ...
  - Phase 2 引入 `workspace_models.toml` + `llmw model` 命令后，再设计"如何将 model 通知给 Claude Code"机制（候选：`.claude/settings.json`）
...
[llmw] cmd:
  claude \
    --add-dir /home/user/yzr_llm_workspace/llm-systems \
    --system-prompt "$(cat /home/user/yzr_llm_workspace/llm-systems/CLAUDE.md)"
...
- **Phase 2**：通过 `workspace_models.toml` + `llmw model` 命令管理 model，并通过 `.claude/settings.json`（或同等机制）将 model 通知 Claude Code
