...
description: llmw wiki enter 的窗口启动模型——agent 开成当前 tmux session 的窗口，复用/打标/status 判定/孤儿清理的规则与坑
metadata:
  type: project
---
...
- **tmux 外选路**（`byobu.visible_sessions`）：恰 1 个可见 session → 直接在其中开窗
  ...
  主机上造出第二个 session，把裸 byobu 直达变成菜单（2026-09-05：用户在 tmux 外跑了
  enter，该假设被实际用法绕过）。
- **复用四条件**：窗口名精确匹配 AND `@llmw_wiki`==wiki AND `@llmw_backend`==当前 backend AND
  ...
  stop 或 --window-suffix**（复用会吞掉"切换 agent"意图）；防劫持同名非 llmw 窗口，
  归属从不从名字反解。
...
- **status 判定链**：BACKEND=`@llmw_backend`（老窗口 fallback `pane_current_command`）；
...
- **孤儿清理**：workspace 目录被删后 status 降级孤儿清理模式——仅默认路径触发，warning + 列表 +
