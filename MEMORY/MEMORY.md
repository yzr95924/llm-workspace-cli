...
- **MEMORY/ 写入与治理归 yzr-memory-management（2026-09-26 终态）** — 两 skill 正文零记忆工作流（「Memory」节已删；`write memory add` 仅存于 wiki 原则 2 写路由表）；落盘机制自承载实例：wiki = fixture `memory-index.txt` 头部声明（`write memory add` 建文件挂索引），workspace = AGENTS.md「Memory 纪律」节（本就含直写 + 索引义务，补「只动 MEMORY/」一行）；判归走 boundary-map V5 补充行；bump = wiki 0.43.6 / workspace 0.10.2
