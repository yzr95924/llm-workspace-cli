...
- [三方边界关系图](boundary-map.md) — 四方（用户 / CLI / wiki skill / workspace 维护 agent）依赖方向 + 生命周期 + 产物归属 + 新能力判归测试；新增归属决策 / 改 skill 文档 / 模板 / CLI 指令文本时先查
...
- **MEMORY/ 写入与治理归 yzr-memory-management（2026-09-26 终态）** — wiki skill 正文零记忆工作流（「Memory」节已删；`write memory add` 仅存于 wiki 原则 2 写路由表）；落盘机制自承载实例：wiki = fixture `memory-index.txt` 头部声明（`write memory add` 建文件挂索引），workspace = 根 AGENTS.md「Memory 纪律」节（本就含直写 + 索引义务，补「只动 MEMORY/」一行）；判归走 boundary-map V5 补充行
