...
- **skill 引用 AGENTS.md 只用稳定锚点（节名 / 字段名 / landmark），禁节号** — wiki / workspace 两个 SKILL 都按此约束：模板内重组（节号重排 / 段落改写）不应让 skill 侧断链，引用改用节名 / 字段名 / landmark 字符串。gate 面 7 守护：7a 扫 CONTRACT_MDS 抓 `AGENTS.md[^\w\n]{0,6}§N` 字面模式（禁节号引用，零误报；`AGENTS.md + [`ref.md`](...) §二` 不触发因 § 与字面 AGENTS.md 间夹杂 word char）；7b 断言 LANDMARKS 列表（wiki 7 个 / workspace 5 个——节名 + @import 链 + 字段名）在模板中出现。**保证机制**：模板内部重组不动 landmark → skill 零修改零感知；动 landmark → CI 红，同 commit 处理（LANDMARKS 列表既是耦合面的显式文档，也是 CI 断言清单；模板改了须同步 skill 引用 + 列表）。模板自身"本文件 §N"自引用（用"本文件"字面量）不触发 7a，随模板重排自然同步
...
- **改 CLI 外部契约必同步 skill + 模板 + 仓根文档** — 子命令 / flag / finding 名 / JSON 字段 / 终态词变更的 commit，必须同 commit grep `yzr-llm-wiki-management/` + `yzr-llm-workspace-management/` + `llmw/content/templates/**/*.md|*.txt` + 仓根 `AGENTS.md`/`CLAUDE.md`/`README.md` 并同步（当前绑定面：命令 153 处 + finding 36+42 + 终态字段 7 + rule_ref 59 + landmark 12 + 布局 token 8）。skill / 模板 / 仓文档与 CLI 同仓共演进，不建抽象间接层（skill 的存在理由是给 agent 精确可执行命令，间接层降执行质量）；只绑 CLI 外部契约，不绑模块内部路径（`llmw.content.*`），模块路径属实现布局、拆模块即静默失效。语法面（命令/flag/风格/跨行断命令/裸 semver）+ 模板依赖（节号禁令/landmark）+ 布局 token 由 `scripts/test/check_skill_cli_contract.py` CI gate 兜底（7 类检查，扫描域含模板 + 仓文档；README 风格的 `[--X\|-Y]` OR 写法也识别；裸 semver 面豁免 `wiki_format_version:` / `workspace_format_version:` 键行 + `upgrade-workflow.md` 整文件）；语义面（行为描述如"只扫不修"）gate 管不到，靠本纪律人工保证。有意不扫 `MEMORY/`（历史命令 + 反例必误报）与 `tests/`（可执行测试自带 loud failure）
