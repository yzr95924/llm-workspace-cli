...
- **改 CLI 外部契约必同步 skill + 模板 + 仓根文档** — 子命令 / flag / finding 名 / JSON 字段 / 终态词变更的 commit，必须同 commit grep `yzr-llm-wiki-management/` + `yzr-llm-workspace-management/` + `llmw/content/templates/**/*.md|*.txt` + 仓根 `AGENTS.md`/`CLAUDE.md`/`README.md` 并同步（当前绑定面：命令 153 处 + finding 36+42 + 终态字段 7 + rule_ref 60）。skill / 模板 / 仓文档与 CLI 同仓共演进，不建抽象间接层（skill 的存在理由是给 agent 精确可执行命令，间接层降执行质量）；只绑 CLI 外部契约，不绑模块内部路径（`llmw.content.*`），模块路径属实现布局、拆模块即静默失效。语法面（命令/flag/风格/跨行断命令/裸 semver）由 `scripts/test/check_skill_cli_contract.py` CI gate 兜底（扫描域含模板 + 仓文档；README 风格的 `[--X\|-Y]` OR 写法也识别；裸 semver 面豁免 `wiki_format_version:` / `workspace_format_version:` 键行 + `upgrade-workflow.md` 整文件）；语义面（行为描述如"只扫不修"）gate 管不到，靠本纪律人工保证。有意不扫 `MEMORY/`（历史命令 + 反例必误报）与 `tests/`（可执行测试自带 loud failure）
