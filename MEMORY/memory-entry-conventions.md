...
**Why 仓库 MEMORY 而非个人 memory：** 项目级记忆散落个人 memory 目录无法被协作方看见、不随代码历史回溯（丢失上下文=丢失设计意图）；所有项目级规则必须**只**放在本仓 `MEMORY/`，会话 memory 只留指针。

**不写什么：** 跟代码可查的事实（命名约定、目录结构、git 历史）不写记忆——记忆只放"代码查不到的为什么"。
...
**本仓实例**：
- 完整：`memory-entry-conventions`（讲"为什么这么分类 / 判别尺度"——自指条目）
...
- **不删除既有条目**——踩坑与决策沉淀下来；如果内容错了，**追加新条目驳正**，不动原文（保留 git 历史）；**例外**：已被代码/文档吸收或过时的事实条目，定期收敛时可删（git 可恢复）
