...
3. **引名不引值**：prose 只引分支 / 定位所需的名字；阈值与字面量归 CLI 输出
4. **finding 口径零镜像**：skill prose 禁 finding 清单（`` `name`（severity`` 格式，gate 面 2 判红）；finding 名可现于分支判定（如 upgrade 触发），但含义 / 修法一律走 `--explain`
5. **定期语义审计**：gate 管语法面（名字 / 指针 / 布局），"行为描述是否仍真实"只能审——大功能收尾或攒批改动后，逐条把文档 claim 钉到代码（本仓 2026-09-19 三轮审计实证抓到 4 处真错）

**已验证承载**：lint-checklist 284→145（改名 lint-workflow——deterministic 清单删、agent 半定性方法论留）；page-templates 476→282（删 YAML / lint 校验镜像，判断内容留）；severity 漏配类 bug 由 `tests/test_findings_registry.py` 双向穷举结构性消灭。
