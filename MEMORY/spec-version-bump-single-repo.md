...
description: spec 版本号 SSOT = llmw 包内常量；bump 仅当现有实例需要 reconcile（模板/fixture/schema 漂移），纯文档变化不 bump
metadata:
  type: project
---
...
frontmatter 的 `*_spec_version`（同 commit，顺序任意）；③ 演进叙事写进 commit message。
