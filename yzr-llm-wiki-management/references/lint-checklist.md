...
### frontmatter 来源（source / synthesis 页）
...
- `sources-out-of-root`（error）：sources 路径解析后不在 wiki 根下（路径穿越）——修法：改
  wiki 根相对路径
- **`raw/external/<symlink>/...` 例外**（详见 external-repo.md「sources: 元素类型」）：以 `raw/external/` 起始时走
...
- `source-in-discussions`（error，仅 source 页）：指向 `raw/discussions/` = provenance
  后门。修法：走 ingest-workflow.md「raw/discussions/ 草稿消化」的归档路径（消化式 / 转正式 `mv` 后再 ingest）
